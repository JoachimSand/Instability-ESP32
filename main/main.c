...
void app_main(void)
{
	...
	rover_pos.x = 0;
	rover_pos.y = 3 * GRID_NODE_SIZE * FORWARD_CNT_PER_CM;
    rover_pos.squal = 0;
	...
	while (1)
	{
        ...
        else // MOTION ROTATE
        {
            ...
            {
                if (rotation_type == ROTATION_90_LEFT)      motor_rotate_in_place(DIR_LEFT,  saturate_uint8_to_val(controller_rotate.output, MAX_ROTATION_VEL));
                if (rotation_type == ROTATION_90_RIGHT)     motor_rotate_in_place(DIR_RIGHT, saturate_uint8_to_val(-controller_rotate.output, MAX_ROTATION_VEL));
                if (rotation_type == ROTATION_180)          motor_rotate_in_place(DIR_RIGHT, saturate_uint8_to_val(controller_rotate.output, MAX_ROTATION_VEL));
            }
        }
